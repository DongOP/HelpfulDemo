 ...
 * 点击按钮可显示或隐藏密码
 */
public class PasswordEditText2 extends EditText {

    private static final String TAG = "DEBUG-WCL: " + PasswordEditText2.class.getSimpleName();

    // 模式的显示图标
    private int mShowPwdIcon = R.drawable.set_show_password;

    // 模式的加密图标
    private int mHidePwdIcon = R.drawable.set_hide_password;
    ...
    private void showPasswordVisibilityIndicator(boolean shouldShowIcon) {
        if (shouldShowIcon) {
            Drawable drawable = mIsShowPwdIcon ?
                    getResources().getDrawable(mHidePwdIcon) :
                    getResources().getDrawable(mShowPwdIcon);
